# શાર્દૂલ શિશુ વિહાર — શિક્ષક માર્ગદર્શિકા

Teacher's weekly planning guide for Shardul Shishu Vihar, Vadodara. Static site hosted on GitHub Pages.

**Live:** https://shardul-vadodara.github.io/shikshak-space/

## What it shows

The week's teacher guide (શિક્ષક માર્ગદર્શિકા), browsable by week and age group:

- **સાવજ-કેસરી** (૨.૫-૫.૫ વર્ષ) and **મૃગેન્દ્ર-પુંડરિક** (૫.૫-૮ વર્ષ)
- Per week: title, dates, and teacher guide downloads (PDF / PNG) — the guide
...
Content is authored in the private `active-parents` repo using the unified planner tool (v4). The teacher clicks **શિક્ષક માર્ગદર્શિકા PDF/PNG સેવ કરો** which writes the guide PDF/PNG to `exports-teachers/`, then commits and pushes. The `publish-teachers.yml` GitHub Action (separate from the parent-portal one) builds the index and pushes it here:

```
data/config.json   school info + groups
data/weeks.json    published weeks index (generated — do not hand-edit)
files/             teacher guide PDFs / PNGs (generated)
logo.png           school logo (header + favicon)
```
